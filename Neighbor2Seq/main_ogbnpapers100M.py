...
parser.add_argument('--K', type=int, default=1)  ### length of sequence we want to use (K<=P)
parser.add_argument('--P', type=int, default=3)  ### length of path we considered during precomputing (K<=P)
...
data = torch.load(os.path.join(args.save_precomputed_data_path, 'ogbn-papers100M_transductive_P' + str(args.P)+'.pt'))
...
print("======================================")
print("=====Total number of nodes in ogbn-papers100M:", len(train_dataset)+len(valid_dataset)+len(test_dataset), "=====")
print("=====Total number of training nodes in ogbn-papers100M:", len(train_dataset), "=====")
print("=====Total number of validation nodes in ogbn-papers100M:", len(valid_dataset), "=====")
print("=====Total number of test nodes in ogbn-papers100M:", len(test_dataset), "=====")
print("======================================")
...
def train(train_loader, evaluator):
    ...
    for sequence_feature, y in train_loader:
        ...
        loss_op(logits, y.to(dtype=torch.int64)).backward()
        optimizer.step()
