...
parser.add_argument('--pe_drop', type=float, default=0)  ### dropout rate for positional encoding
parser.add_argument('--kernel_size', type=int, default=7)  ### conv kernel size
parser.add_argument('--batch_size', type=int, default=256)
parser.add_argument('--vt_batch_size', type=int, default=50000)  ### batch size used for val/test
parser.add_argument('--log_step', type=int, default=100)
parser.add_argument('--transductive', type=bool, default=False)  ### transductive setting or inductive setting
parser.add_argument('--multilabel', type=bool, default=False)   ### multilabel classification or singlelabel classification
...
if args.transductive:
    ...
    data = torch.load(os.path.join(args.save_precomputed_data_path, args.dataset + '_transductive_P' + str(args.P)+'.pt'))
else:
    print('===Inductive learning===')
    data = torch.load(os.path.join(args.save_precomputed_data_path, args.dataset + '_inductive_P' + str(args.P)+'.pt'))

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
num_node_features = int(data.sequence_feature.shape[-1])
if args.multilabel:
    num_classes = int(data.y.shape[-1])
else:
...
print("======================================")
print("=====Total number of nodes in", args.dataset ,":", len(train_dataset)+len(valid_dataset)+len(test_dataset), "=====")
print("=====Total number of training nodes in", args.dataset ,":", len(train_dataset), "=====")
print("=====Total number of validation nodes in", args.dataset ,":", len(valid_dataset), "=====")
print("=====Total number of test nodes in", args.dataset ,":", len(test_dataset), "=====")
print("======================================")
...
def train(train_loader, multilabel):
    model.train()
    y_true, y_pred = [], []
    for sequence_feature, y in train_loader:
        sequence_feature, y = sequence_feature.to(device), y.to(device)
        optimizer.zero_grad()
        ...
        y_pred.append(pred)
        y_true.append(y)
    y_trues = torch.cat(y_true, dim=0)
    y_preds = torch.cat(y_pred, dim=0)
    micro_f1 = calc_f1(y_trues.cpu().detach().numpy(), y_preds.cpu().detach().numpy())
    return micro_f1


@torch.no_grad()
def test(loader, multilabel):
    model.eval()
    y_true, y_pred = [], []
...
for run in range(1, args.runs+1):
    ...
    for epoch in range(1, args.epochs+1):
        train_result = train(train_loader, args.multilabel)
        val_result = test(valid_loader, args.multilabel)
        if val_result > best_val_result:
            best_val_result = val_result
            test_result = test(test_loader, args.multilabel)
            best_epoch = epoch 
